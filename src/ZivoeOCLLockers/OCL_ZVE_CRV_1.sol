...
contract OCL_ZVE_CRV_1 is ZivoeLocker {
    ...
    address public constant _3CRV_LP = 0xbEbc44782C7dB0a1A60Cb6fe97d0b483032FF1C7;      /// @dev 3CRV 3Pool (DAI/USDC/USDT) for CRV Finance.
    ...
    constructor(
        ...
        ZVE_MP = ICRVDeployer(CRV_Deployer).deploy_metapool(
            _3CRV_LP,                   /// The base-pool (3CRV = 3Pool).
    ...
    function pushToLockerMulti(address[] calldata assets, uint256[] calldata amounts) public override onlyOwner {
        ...
        if (assets[0] == DAI) {
            IERC20(DAI).approve(_3CRV_LP, IERC20(DAI).balanceOf(address(this)));
            uint256[3] memory deposits_bp;
            deposits_bp[0] = IERC20(DAI).balanceOf(address(this));
            ICRVPlainPool3CRV(_3CRV_LP).add_liquidity(deposits_bp, 0);
        }
        else if (assets[0] == USDC) {
            IERC20(USDC).approve(_3CRV_LP, IERC20(USDC).balanceOf(address(this)));
            uint256[3] memory deposits_bp;
            deposits_bp[1] = IERC20(USDC).balanceOf(address(this));
            ICRVPlainPool3CRV(_3CRV_LP).add_liquidity(deposits_bp, 0);
        }
        else {
            IERC20(USDT).approve(_3CRV_LP, IERC20(USDT).balanceOf(address(this)));
            uint256[3] memory deposits_bp;
    ...
    function pullFromLockerMulti(address[] calldata assets) public override onlyOwner {
        ...
        require((assets[0] == DAI || assets[1] == USDC || assets[1] == USDT) && assets[2] == IZivoeGBL(GBL).ZVE());
    }
