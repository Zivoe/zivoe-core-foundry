...
contract OCR_CycleV2 is ZivoeLocker, ReentrancyGuard {
    ...
    /// @notice Initializes the OCR_CycleV2 contract.
    ...
    constructor(
        ...
        require(_redemptionFeeBIPS <= 1000, "OCR_CycleV2::constructor() _redemptionFeeBIPS > 1000");
    ...
    modifier isUnderwriter() {
        require(_msgSender() == underwriter, "OCR_CycleV2::isUnderwriter() _msgSender() != underwriter");
        _;
    }
    ...
    function pushToLocker(
        ...
        require(asset == USDC, "OCR_CycleV2::pushToLocker() asset != USDC");
        
        // Transfer USDC from DAO to this locker
    ...
    function pullFromLocker(address asset, bytes calldata data) external override onlyOwner nonReentrant {
        require(asset == USDC, "OCR_CycleV2::pullFromLocker() asset != USDC");

        // Transfer all USDC from this locker to DAO
    ...
    function pullFromLockerPartial(
        ...
        require(asset == USDC, "OCR_CycleV2::pullFromLockerPartial() asset != USDC");
        
        // Transfer a portion of USDC from this locker to DAO
    ...
    function calculateRedemptionAmount(uint256 zVLTAmount) external view returns (uint256 usdcAmount, uint256 fee) {
        require(zVLTAmount > 0, "OCR_CycleV2::calculateRedemptionAmount() zVLTAmount == 0");
    ...
    function redeemUSDC(uint256 zVLTAmount) external nonReentrant {
        require(zVLTAmount > 0, "OCR_CycleV2::redeemUSDC() zVLTAmount == 0");
    ...
    function updateRedemptionFeeBIPS(uint256 _redemptionFeeBIPS) external {
        require(
            ...
            "OCR_CycleV2::updateRedemptionFeeBIPS() _msgSender() != ZVL()"
        );
        require(
            _redemptionFeeBIPS <= 1000, "OCR_CycleV2::updateRedemptionFeeBIPS() _redemptionFeeBIPS > 1000"
        );
    ...
    function cycle(uint256[] calldata amounts, address[] calldata users) external isUnderwriter {
        require(amounts.length == users.length, "OCR_CycleV2::cycle() amounts.length != users.length");
        uint totalAmount;
