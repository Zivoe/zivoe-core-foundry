...
contract OCR_ModularV2 is ZivoeLocker, ReentrancyGuard {
    ...
    event RequestProcessed(uint256 indexed id, address indexed account, uint256 burnAmount, uint256 redeemAmount, bool indexed seniorElseJunior);
    ...
    function destroyRequest(uint256 id) public _tickEpoch {
        require(requests[id].account == _msgSender(), "OCR_Modular::destroyRequest() requests[id].account != _msgSender()");
    ...
    function processRequest(uint256 id) public _tickEpoch {
        require(requests[id].amount > 0, "OCR_Modular::destroyRequest() requests[id].amount == 0");
        uint256 totalRedemptions = redemptionsAllowedSenior * (BIPS - epochDiscountSenior) + redemptionsAllowedJunior * (BIPS - epochDiscountJunior);
    ...
    function tickEpoch() public {
        if (block.timestamp > epoch) { 
            ...
            emit EpochTicked(epoch, redemptionsAllowedJunior, redemptionsAllowedSenior, epochDiscountJunior, epochDiscountSenior);
